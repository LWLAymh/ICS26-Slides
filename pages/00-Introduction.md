...
- `rm qwq.txt`: 删除名为"qwq.txt"的文件
  ...
  - `rm -rf qwq`: 强制递归删除名为"qwq"的文件夹（危险操作，执行前务必确认路径）
...
- `ps aux`: 显示系统中的进程及其详细信息
  - `ps -ef`: 以全格式显示所有进程
  - `ps -eH`: 以树形结构显示所有进程
- `bg %1`: 让作业号为 1 的已暂停作业在后台继续运行（可先用 `Ctrl+Z` 暂停前台作业）
- `fg %1`: 将作业号为 1 的作业调回前台
- `kill 114514`: 向 pid 为 114514 的进程发送信号；默认发送 `SIGTERM`
  - 对前台进程按 `Ctrl+C` 会发送 `SIGINT`，进程可以选择捕获或忽略该信号
...
- 新建节点
  ...
  - 你也许听说过公钥密码系统：它使用一对相互关联的公钥和私钥。SSH 公钥认证中，需要妥善保护私钥，并将公钥提供给服务器。
...
- 配置你的远端(方法A)
  ...
  - 当你发现终端中显示`xxxx:end`时, 点击VSCode中的"新建终端", 此时您已经配置成功. 建议您在目录 `~/ics-2026` 下为每个lab创建相应的文件夹.
