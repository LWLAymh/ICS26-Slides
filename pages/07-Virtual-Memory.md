...
layout: cover
coverBackgroundUrl: /07-Virtual-Memory/cover.jpg
---
# Virtual Memory
...
![physical_addressing](/07-Virtual-Memory/physical_addressing.png){.h-55.mx-auto}
...
![virtual_addressing](/07-Virtual-Memory/virtual_addressing.png){.h-55.mx-auto}
...
![memory_hierarchy](/07-Virtual-Memory/memory_hierarchy.png){.mx-auto}
...
![vm_as_cache](/07-Virtual-Memory/vm_as_cache.png){.mx-auto}
...
![page_table](/07-Virtual-Memory/page_table.png){.h-60.mx-auto}
...
![page_table](/07-Virtual-Memory/page_table.png){.h-70.mx-auto}
...
![review_fully_associative](/07-Virtual-Memory/review_fully_associative.png){.mx-auto}
...
**分配页面**：调用 `malloc` 会先取得一段虚拟地址空间；首次访问其中的页面时，内核再分配物理页并更新对应的页表条目（PTE）。
...
程序通常集中访问工作集中的页面，因此虚拟内存能够利用时间局部性；连续地址访问也能利用页内的空间局部性。
...
![private_address_space](/07-Virtual-Memory/private_address_space.png){mx-auto}

</div>
<div :class="$clicks==2 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute>

![page_table](/07-Virtual-Memory/page_table.png){mx-auto}

</div>
<div :class="$clicks>=3 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute>

![vm_share_page](/07-Virtual-Memory/vm_share_page.png){mx-auto}
...
![mem_protection](/07-Virtual-Memory/mem_protection.png){.h-70.mx-auto}
...
![vm_translation](/07-Virtual-Memory/vm_translation.png){.mx-auto}
...
- VPN 直接作为索引去查找，替代了标记位的对比判断，经页表查询后得到物理页号 PPN <br>（PPN $\times$ 页大小就是物理页的起始地址）
- VPO 等价于块偏移，用于索引页内地址
- PA = PPN : PPO
...
**此时还没有使用 TLB，因此地址翻译需要先访问页表，再访问目标数据。**{.text-sky-5}

</div>

<div>

![vm_translation_detail_hit](/07-Virtual-Memory/vm_translation_detail_hit.png){.mx-auto}
...
**能够读到页表项不代表目标页面在物理内存中；是否在内存中由 PTE 的有效位判断。**{.text-sky-5}

</div>

<div>

![vm_translation_detail_miss](/07-Virtual-Memory/vm_translation_detail_miss.png){.mx-auto}
...
![page_table_cache](/07-Virtual-Memory/page_table_cache.png){.mx-auto.h-70}
...
- 利用的是虚拟地址的局部性
- TLB 缓存虚拟地址到物理地址的映射，对应 PTE，目的是加速 VP 到 PP 的翻译
...
![tlb](/07-Virtual-Memory/tlb.png){.mx-auto}
![vp_tlb](/07-Virtual-Memory/vp_tlb.png){.mx-auto}
...
![vp_tlb](/07-Virtual-Memory/vp_tlb.png){.mx-auto}
![cache_address](/07-Virtual-Memory/cache_address.png){.mx-auto}
...
TLB 利用了地址翻译的局部性：连续访问同一虚拟页时，VPN 不变而 VPO 变化；跨页连续访问时，VPN 通常相邻。

另一方面，虚拟地址空间通常很稀疏，能不能避免为未使用的区域保存大量页表项呢？
...
![multi_level_page_table](/07-Virtual-Memory/multi_level_page_table.png){.mx-auto}


</div>
<div :class="$clicks==1 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute>

![multi_level_page_table_2](/07-Virtual-Memory/multi_level_page_table_2.png){.mx-auto}
...
![multi_level_page_table](/07-Virtual-Memory/multi_level_page_table.png){.mx-auto.h-50}


![multi_level_page_table_2](/07-Virtual-Memory/multi_level_page_table_2.png){.mx-auto.h-50}
...
- Cache 缓存**物理地址**，对应以物理地址开头的物理块，目的是加快数据字的访问
  - 思考：为什么 Cache 缓存的不是*虚拟地址*
...
- 多级页表前 $k-1$ 级的地址字段是 **下一级页表**{.text-sky-5} 的物理页基址，第 $k$ 级的地址字段是虚拟页对应的 **物理页**{.text-sky-5} 的基址
...
![address_translation_all](/07-Virtual-Memory/address_translation_all.png){.mx-auto}
...
- **大页位**（PS，Page Size），中间层页表项可以直接映射大页；第三级可映射 2MB 大页，第二级可映射 1GB 大页
- **引用位**（A，Accessed），表示该页是否被访问过，**由 MMU 在访问时设置，由软件清除，为了实现替换策略**{.text-sky-5}
- **脏位**（D，Dirty），表示该页是否被修改过，**由 MMU 在写入时设置，由软件清除，为了确定是否要写回**{.text-sky-5}
...
![i7_p1_to_p3](/07-Virtual-Memory/i7_p1_to_p3.png){.mx-auto.h-100}

</div>
<div :class="$clicks==1 ? 'opacity-100' : 'opacity-0'" transition duration-400 absolute w-full>

![i7_p4](/07-Virtual-Memory/i7_p4.png){.mx-auto.h-100}
...
![linux_vm_process](/07-Virtual-Memory/linux_vm_process.png){.mx-auto.h-100}
...
- `pgd`：page global directory，指向第一级页表（页全局目录）的基址；该页表的物理基址会装入 CR3 寄存器
...
![linux_vm_organization](/07-Virtual-Memory/linux_vm_organization.png){.mx-auto.h-90}
...
3. 如果上述检查均通过，则确定了这个缺页是由于对合法的虚拟地址进行合法的操作造成的
    <br> → 执行正常的缺页处理
    1. 处理程序分配一个空闲物理页；必要时选择牺牲页，并在牺牲页被修改后将其写回；
    2. 从后备存储载入页面内容（或进行零初始化），再修改对应的页表；
    3. 从处理程序返回

<!--
当 MMU 翻译虚拟地址 A 时遇到 page fault，page fault 处理程序怎么处理
-->
...
![page_fault](/07-Virtual-Memory/page_fault.png){.mx-auto.h-90}
...
**内存共享**：多个进程的虚拟内存区域可以映射到相同的物理页，从而节省物理内存。

**文件与内存同步**：共享映射中的修改可以写回文件；私有映射中的修改不会写回原文件。

**按需调页**：只在访问页面时读入相应内容，并省去显式的 `read` / `write` 操作。
...
2. 匿名页或私有映射中被修改的页不能写回原文件；被换出时，需要保存到交换空间。
3. 共享文件映射中的脏页则可以写回它所映射的文件。
...
![shared_object](/07-Virtual-Memory/shared_object.png){.mx-auto}
...
![cow](/07-Virtual-Memory/cow.png){.mx-auto}
...
3. 父子进程中可写的私有页被设置为 **只读并标记为写时复制**{.text-sky-5}；共享映射仍保持共享
4. 当父子进程都没有对页进行修改时，父子进程是共享相同的物理内存的
5. 当其中一个进程对页进行修改时，就会对该页进行写时复制，并为该页赋予写权限，并更新进程对应的页表
...
![loader](/07-Virtual-Memory/loader.png){.mx-auto.h-100}
...
![mmap](/07-Virtual-Memory/mmap.png){.mx-auto}
...
C 程序中需要额外的虚拟内存的时候，一般使用动态内存分配器（dynamic memory allocator）

分配器维护着**运行时堆**，堆顶指针 `brk`，将堆视作一组不同大小的**块**的集合。块要么**已分配**，要么**空闲**。

分配器的两种风格：

- **显式分配器**（explicit allocator）：要求应用显式释放已分配的块（本节讨论）：C 中的 `malloc` 和 `free`，C++ 中的 `new` 和 `delete`
- **隐式分配器**（implicit allocator）：分配器检测不再被使用的已分配块并自动释放之。又被称作垃圾收集器（garbage collector）。Lisp、ML、Java、Python 等语言。

</div>

<div>

![mmap](/07-Virtual-Memory/heap.png){.mx-auto}
...
`void *malloc(size_t size)` 返回指向大小至少为 `size` 字节的内存块。可能会做对齐（32 位对齐 8，64 位对齐 16）

`calloc` 将分配的内存初始化为 0。

`realloc` 改变一个以前已分配块的大小。

`malloc` 可以通过 `mmap` 和 `munmap` 获得或归还虚拟内存，也可以用 `sbrk` 扩展堆。
...
- 最大化吞吐率
  - 即单位时间内完成的请求数
  - 合理性能：一个分配请求的最坏时间与空闲块数量呈线性关系，且可以 $O(1)$ 完成释放请求
- 最大化内存利用率
  - 即高效使用堆空间
  - 最大化峰值利用率
  - 碎片
    - 内部碎片：已分配块大于有效载荷
    - 外部碎片：空闲内存之和可满足请求，但没有单独块可满足请求（取决于将来请求的模式）
...
脚标记，在块的末尾添加一个头部的副本，相当于在块尾也标记块大小。*内部碎片*

</div>

<div v-click>
去脚优化：只有空闲块需要脚部（想想为什么）
</div>
...
![mmap](/07-Virtual-Memory/implicit_block.png){.mx-auto}

</div>
</div>

<!--
first fit：从头开始找到第一个适配的

next fit：从上一次结束查询的地方开始
...
- 缺点：增大内部碎片
- 维护策略：
  - 后进先出（LIFO）：新释放的块放置在链表开始处，其与 first fit 策略适配，$O(1)$ 释放、合并。
  - 按地址顺序：链表中每个块的地址小于后继地址，$O(n)$ 找前驱，但是 util 更高（first fit 接近 best fit）
  
</div>

<div>

![](/07-Virtual-Memory/explicit_block.png)
...
- 维护多个空闲链表，每个链表中的块大小大致相同，比如说按照 2 的幂次
- 简单分离存储：
  - 每个大小类的空闲链表包含大小相同的块，每个块的大小为大小类中最大元素大小
  - 不会合并、分割空闲块，造成内部碎片和外部碎片
- 分离适配：
  - 每个大小类维护一个显式/隐式链表
  - GNU `malloc` 包就是这样实现的
  - 伙伴系统：分离适配的一种特例（P605）
...
- 野指针：
  - 指针未初始化
  - 指针所指向的内存被释放
  - 指针超出变量的作用域：`return &val`，其中 `val` 为局部变量
  - ……（欢迎举例）
- 间接引用坏指针：`scanf("%d", n)`
- 读未初始化内存：`malloc` 初始化的东西不为 0
- 栈缓冲区溢出：`gets(buf)`
- 引用成指针而不是其指向的对象：`*ptr--` 注意优先级
- 误解指针运算：指针是以其指向对象的大小为单位进行运算的（不一定是字节）
- 内存泄漏：申请了但是不释放
