...
layout: cover
coverBackgroundUrl: /04-Memory-Hierarchy/cover.jpg
---
# Memory Hierarchy
...
![sram](/04-Memory-Hierarchy/sram.png){.h-40.mx-auto}
...
![dram](/04-Memory-Hierarchy/dram.png)
...
![dram_core](/04-Memory-Hierarchy/dram_core.png){.h-100px.mx-auto}
...
![dram_supercell](/04-Memory-Hierarchy/dram_supercell.png){.h-100px.mx-auto}
...
![dram](/04-Memory-Hierarchy/dram.png){.h-100px.mx-auto}
...
![dram_ras_cas](/04-Memory-Hierarchy/dram_ras_cas.png){.mx-auto.h-60}
...
![dram_example](/04-Memory-Hierarchy/dram_example.png)
...
![sdram](/04-Memory-Hierarchy/sdram.png){.w-70.mx-auto}
...
- SDRAM 的增强版本，在时钟上升沿和下降沿各传输一次数据
- 相同时钟频率下，数据传输率翻倍

![ddrm](/04-Memory-Hierarchy/ddrm.png){.w-70.mx-auto}
...
![disk_structure](/04-Memory-Hierarchy/disk_structure.png){.mx-auto.h-60}
...
![disk_traditional](/04-Memory-Hierarchy/disk_traditional.svg){.mx-auto.h-40}

![disk_multi_zone](/04-Memory-Hierarchy/disk_multi_zone.svg){.mx-auto.h-40}
</div>
...
1. **寻道**：通过传动臂将读写头移动到对应磁道上（$T_{\text{seek}}$）
...
![disk_seek](/04-Memory-Hierarchy/disk_seek.png){.h-40.mx-auto}
...
![disk_rotate](/04-Memory-Hierarchy/disk_rotate.png){.h-40.mx-auto}
...
![ssd](/04-Memory-Hierarchy/ssd.png)
...
![memory_hierarchy](/04-Memory-Hierarchy/memory_hierarchy.png)
...
D. 旋转磁盘相比 SSD 仍然有单位容量价格低、没有闪存擦写次数限制等优点
...
![miss](/04-Memory-Hierarchy/miss.svg)
...
![thrashing](/04-Memory-Hierarchy/thrashing.svg){.mx-auto.h-60}
...
![cache_arch](/04-Memory-Hierarchy/cache_arch.png)
...
![cache_address](/04-Memory-Hierarchy/cache_address.png)
...
4. **行替换**：否则，发生缓存不命中，优先使用无效行；没有无效行时再选择一行驱逐。
...
![cache_set_index](/04-Memory-Hierarchy/cache_set_index.png)

![cache_byte_index](/04-Memory-Hierarchy/cache_byte_index.png)
...
![cache_set_index_pos](/04-Memory-Hierarchy/cache_set_index_pos.png)
...
![cache_direct_mapped](/04-Memory-Hierarchy/cache_direct_mapped.png){.w-60}

![cache_set_associative](/04-Memory-Hierarchy/cache_set_associative.png){.w-60}

![cache_fully_associative](/04-Memory-Hierarchy/cache_fully_associative.png){.w-60}
...
- 写分配（Write Allocate）：先把包含该地址的块加载到缓存，再写缓存
...
块大小（$B$）：
- 块越大，越能利用空间局部性
- 容量不变时，块越大，缓存行数越少，而且不命中时需要传送的数据更多
...
- $E$ 越高，复杂性越高、成本越高
- $E$ 越高，命中时需要比较的标记越多，替换逻辑也更复杂
- $E$ 增高，可能需要更多标记位（$t \geq \log_2 E$）、LRU 状态位
- 原则是命中时间和不命中率的折中{.text-sky-5}
...
![memory_hill](/04-Memory-Hierarchy/memory_hill.png){.h-100.mx-auto}
...
![memory_hill_spatial_locality](/04-Memory-Hierarchy/memory_hill_spatial_locality.png)
...
- 工作集较小时，数据容易装入上级缓存，缓存命中率较高。
- 工作集增大并超过某一级缓存容量时，容量不命中增多，需要从更低层存储器读取，传输速率下降。

</div>

<div col-span-2>

![memory_hill_temporal_locality](/04-Memory-Hierarchy/memory_hill_temporal_locality.png)
...
![memory_hill_prefetch](/04-Memory-Hierarchy/memory_hill_prefetch.png)
