...
\* 直接引用地址空间中内核区内的代码和数据会导致保护故障，但是可以通过 `/proc` 文件系统访问一部分内核数据结构的内容。
...
**定义**：上下文切换是内核切换所运行进程时，对进程上下文（context）的保存和恢复。
...
1. **系统调用**：进程 A 调用 `read`，通过系统调用陷入内核态（陷阱）
2. **上下文切换**：内核处理系统调用，知道要等很久（DMA 直接内存访问），于是决定不返回到 A，而是切换到 B（抢占、调度）
3. **中断处理程序（handler）**：B 正在运行，磁盘发出中断，告知内核数据已经拿到了，于是内核进入中断处理程序
...
2. 后台进程若想从终端读取输入，会停止直到转为前台进程；终端启用 `TOSTOP` 时，后台写终端也会使进程停止。
...
2. 信号发送给进程 a 后，在被接收前称为待处理信号，记录在进程 a 的 **`pending` 位向量**{.text-sky-5} 中
3. 若该信号未被阻塞，内核会在合适的时机让进程接收它
4. 位向量：每一位对应一个信号，只有 0 和 1 两种状态，0 表示未收到此信号，1 表示收到此信号
5. 进程 a 获得 CPU、内核准备将控制转移到用户态时，可以处理待处理且未被阻塞的信号
...
进程可以通过 `setpgid` 函数改变自己或尚未执行 `execve` 的子进程的进程组。
...
**作业（job）**：由 shell 创建和管理的一个或多个进程；一个作业整体在前台或后台运行。
...
- 如果 `pid < -1`，则将信号 `sig` 发送给进程组 `|pid|` 内的所有进程
- 如果 `pid == -1`，则发送给调用进程有权限发送到的所有进程
...
<kbd>Ctrl+D</kbd> 输入终止字符；终端在当前输入为空时让读取操作得到 EOF（它不是信号）
...
但是，普通信号不会排队；已有同类型信号处于待处理状态时，后续同类信号会合并在同一个待处理位中。
...
2. 只调用 **异步信号安全** 函数 <span class="text-sm text-gray-5">这类函数可以在信号处理程序中安全调用，不会破坏被中断函数的内部状态</span>
3. 保存恢复 `errno`
4. 访问全局数据结构时阻塞所有信号 <span class="text-sm text-gray-5">避免竞争</span>
...
7. 不可以用普通信号来对其他进程中发生的事件计数 <span class="text-sm text-gray-5">因为同类待处理信号不会排队</span>
...
这是因为，`errno == EINTR` 表示这次调用被信号中断，没有读取任何数据，文件位置没有发生变化，我们即将重试此次操作。
...
`rio_writen` 函数用于把缓冲区 `usrbuf` 中的 `n` 个字节写入文件描述符 `fd`。
...
- Unix I/O
  ...
  - `write` **异步信号安全**，在 signal handler 中常用 `write` 输出（回顾第 8 章内容）
- STDIO
  - 由 C 语言定义，在 libc 库中实现
  - 在 Unix I/O 上加了缓冲和流抽象，**减少系统调用的次数**
  - 不适合直接用于套接字上的双向 I/O（见课本 p639）
- RIO
